...
app.post('/get-status', async (req,res) => {
  ...
  let status = savedSessions[sessionIndex].ready 
  res.status(200).json({
    status: status,
    response: "connected"
  });
...
app.post('/get-groups', async (req, res) => {
  ...
  const groups = await client.getChats().then(chats => {
    return chats.find(chat => 
      chat.isGroup 
    );
  });
  console.log("groups:"+JSON.stringify(client.getChats()))
  res.status(200).json({
    status: true,
    response: groups
  });
...
app.post('/send-group-message-media', [
  body('id').custom((value, { req }) => {
    if (!value && !req.body.name) {
      console.log("print throw");
      throw new Error('Invalid value, you can use `id` or `name`');
    }
    return true;
  }),
...
app.post('/free-media-store', async (req,res) => {
  mediaStore = {}
  res.status(200).json({
    status: true,
    response: "freed"
